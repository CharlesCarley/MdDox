...
namespace MdDox
{
    ...
    void PathUtil::construct(const String& str)
    {
        ...
        int64_t i = (int64_t)clean.size() - 1;
        for (; n == NLoc && i >= 0; --i)
        {
            if (clean[i] == '/')
                n = i;
    ...
    {
        return StringCombine(stem(), fullExtension());
    }
    ...
    String PathUtil::parentDir(size_t n) const
    {
        ...
        if (!dir.empty())
        {
            ...
            for (const String& ent : dir)
                oss << ent << '/';
            return StringCombine(_root, '/', oss.str());
        }
        return StringCombine(_root, '/');
    }
    ...
    String PathUtil::fullPath() const
    {
        ...
        if (_directory.empty())
            return StringCombine(_file, ext);

        return StringCombine(_root, '/', _directory, _file, ext);
    }

    String PathUtil::rootedDir() const
    {
        return StringCombine(_root, '/', _directory);
    }
    ...
    void PathUtil::appendDirectory(const PathUtil& filePath)
    {
        if (!filePath._directory.empty())
            _directory = StringCombine((const String&)_directory, filePath._directory);
    }
