...
-------------------------------------------------------------------------------
*/
#include "XRefSectQuery.h"
#include "TypeFilter/DoxygenFilter.h"


namespace MdDox::Doxygen
{
    const String& XRefSectQuery::getId(const String& notFound) const
    {
        if (_node)
    ...
    const String& XRefSectQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void XRefSectQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
