...
{
    const String& CompoundRefQuery::getRefId(const String& notFound) const
    {
        if (_node)
	...
	DoxProtectionKindEnum CompoundRefQuery::getProt() const
	{
		if (_node)
	...
	DoxVirtualKindEnum CompoundRefQuery::getVirt() const
	{
		if (_node)
    ...
    const String& CompoundRefQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void CompoundRefQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
