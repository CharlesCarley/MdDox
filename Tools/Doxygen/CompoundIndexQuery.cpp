...
-------------------------------------------------------------------------------
*/
#include "CompoundIndexQuery.h"
#include "TypeFilter/DoxygenFilter.h"

#include "MemberIndexQuery.h"

namespace MdDox::Doxygen
{
    void CompoundIndexQuery::visit(Visitors::CompoundIndexQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 63:
                visitor->visitedMember(MemberIndexQuery(obj));
                break;
            case 65:
    ...
    const String& CompoundIndexQuery::getRefId(const String& notFound) const
    {
        if (_node)
	...
	DoxCompoundKindEnum CompoundIndexQuery::getKind() const
	{
		if (_node)
    ...
    const String& CompoundIndexQuery::getName(const String& notFound) const
    {
        if (_node) {
    ...
    void CompoundIndexQuery::foreachMember(const MemberIndexQueryFunction& invoke) const
    {
        QueryForEach<MemberIndexQuery, 63>(invoke, _node);
    }


    const String& CompoundIndexQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void CompoundIndexQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
