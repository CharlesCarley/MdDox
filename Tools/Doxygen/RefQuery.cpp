...
{
    const String& RefQuery::getRefId(const String& notFound) const
    {
        if (_node)
	...
	DoxProtectionKindEnum RefQuery::getProt() const
	{
		if (_node)
    ...
    const String& RefQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void RefQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
