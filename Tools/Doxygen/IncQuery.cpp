...
{
    const String& IncQuery::getRefId(const String& notFound) const
    {
        if (_node)
	...
	DoxBoolEnum IncQuery::getLocal() const
	{
		if (_node)
			return (DoxBoolEnum)DoxBool::get(_node->attribute("local"));
		return (DoxBoolEnum)-1; 
	}



    const String& IncQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void IncQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
