...
{
    void EntryQuery::visit(Visitors::EntryQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
	...
	DoxBoolEnum EntryQuery::getThead() const
	{
		if (_node)
			return (DoxBoolEnum)DoxBool::get(_node->attribute("thead"));
		return (DoxBoolEnum)-1; 
	}

    void EntryQuery::foreachParagraph(const ParaQueryFunction& invoke) const
    {
        QueryForEach<ParaQuery, 68>(invoke, _node);
    }


    const String& EntryQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void EntryQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
