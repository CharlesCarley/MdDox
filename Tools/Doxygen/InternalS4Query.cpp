...
{
    void InternalS4Query::visit(Visitors::InternalS4QueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    void InternalS4Query::foreachParagraph(const ParaQueryFunction& invoke) const
    {
        QueryForEach<ParaQuery, 68>(invoke, _node);
    }


    const String& InternalS4Query::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void InternalS4Query::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
