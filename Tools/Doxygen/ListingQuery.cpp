...
{
    void ListingQuery::visit(Visitors::ListingQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 11:
                visitor->visitedCodeLine(CodeLineQuery(obj));
                break;        
            default:
                break;
            }
    ...
    const String& ListingQuery::getFileName(const String& notFound) const
    {
        if (_node)
            return _node->attribute("filename", notFound);
        return notFound;
    }

    void ListingQuery::foreachCodeLine(const CodeLineQueryFunction& invoke) const
    {
        QueryForEach<CodeLineQuery, 11>(invoke, _node);
    }


    const String& ListingQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ListingQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
