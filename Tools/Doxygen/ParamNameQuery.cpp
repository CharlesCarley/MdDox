...
{
    void ParamNameQuery::visit(Visitors::ParamNameQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
	...
	DoxParamDirEnum ParamNameQuery::getDirection() const
	{
		if (_node)
			return (DoxParamDirEnum)DoxParamDir::get(_node->attribute("direction"));
		return (DoxParamDirEnum)-1; 
	}

	void ParamNameQuery::getRef(RefTextQuery &dest) const
	{
		if (_node) {
	...
	RefTextQuery ParamNameQuery::getRef() const
	{
		RefTextQuery dest;
		getRef(dest);
		return dest;
    ...
    const String& ParamNameQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ParamNameQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
