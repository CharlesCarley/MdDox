...
{
    void TemplateParamListQuery::visit(Visitors::TemplateParamListQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    void TemplateParamListQuery::foreachParameter(const ParamQueryFunction& invoke) const
    {
        QueryForEach<ParamQuery, 69>(invoke, _node);
    }


    const String& TemplateParamListQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void TemplateParamListQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
