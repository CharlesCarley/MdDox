...
{
    const String& AnchorQuery::getId(const String& notFound) const
    {
        if (_node)
    ...
    const String& AnchorQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void AnchorQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
