...
{
    int64_t SpQuery::getValue(int64_t def) const
    {
        return _node ? _node->integer("value") : def;
    }



    const String& SpQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void SpQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
