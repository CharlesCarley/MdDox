...
{
    void GraphQuery::visit(Visitors::GraphQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 66:
                visitor->visitedNode(NodeQuery(obj));
                break;        
            default:
                break;
            }
    ...
    void GraphQuery::foreachNode(const NodeQueryFunction& invoke) const
    {
        QueryForEach<NodeQuery, 66>(invoke, _node);
    }


    const String& GraphQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void GraphQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
