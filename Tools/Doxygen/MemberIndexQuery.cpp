...
{
    const String& MemberIndexQuery::getRefId(const String& notFound) const
    {
        if (_node)
	...
	DoxMemberKindEnum MemberIndexQuery::getKind() const
	{
		if (_node)
    ...
    const String& MemberIndexQuery::getName(const String& notFound) const
    {
        if (_node) {
    ...
    const String& MemberIndexQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void MemberIndexQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
