...
{
    void BlockQuoteQuery::visit(Visitors::BlockQuoteQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    void BlockQuoteQuery::foreachParagraph(const ParaQueryFunction& invoke) const
    {
        QueryForEach<ParaQuery, 68>(invoke, _node);
    }


    const String& BlockQuoteQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void BlockQuoteQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
