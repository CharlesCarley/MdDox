...
#include "ListItemQuery.h"

namespace MdDox::Doxygen
{
    void ListQuery::visit(Visitors::ListQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 60:
                visitor->visitedListItem(ListItemQuery(obj));
                break;        
            default:
                break;
            }
    ...
    void ListQuery::foreachListItem(const ListItemQueryFunction& invoke) const
    {
        QueryForEach<ListItemQuery, 60>(invoke, _node);
    }


    const String& ListQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ListQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
