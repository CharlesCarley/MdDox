...
{
    void ListItemQuery::visit(Visitors::ListItemQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    void ListItemQuery::foreachParagraph(const ParaQueryFunction& invoke) const
    {
        QueryForEach<ParaQuery, 68>(invoke, _node);
    }


    const String& ListItemQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ListItemQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
