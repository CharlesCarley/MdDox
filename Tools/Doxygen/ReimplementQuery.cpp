...
{
    const String& ReimplementQuery::getRefId(const String& notFound) const
    {
        if (_node)
    ...
    const String& ReimplementQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ReimplementQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
