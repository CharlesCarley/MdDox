...
{
    void ParamNameListQuery::visit(Visitors::ParamNameListQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 73:
                visitor->visitedParameterName(ParamNameQuery(obj));
                break;        
            default:
                break;
            }
    ...
    void ParamNameListQuery::foreachParameterName(const ParamNameQueryFunction& invoke) const
    {
        QueryForEach<ParamNameQuery, 73>(invoke, _node);
    }


    const String& ParamNameListQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ParamNameListQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
