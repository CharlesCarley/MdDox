...
{
    void LinkedTextQuery::visit(Visitors::LinkedTextQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    void LinkedTextQuery::foreachRef(const RefTextQueryFunction& invoke) const
    {
        QueryForEach<RefTextQuery, 82>(invoke, _node);
    }


    const String& LinkedTextQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void LinkedTextQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
