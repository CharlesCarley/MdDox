...
{
    void ListOfAllMembersQuery::visit(Visitors::ListOfAllMembersQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 63:
                visitor->visitedMember(MemberRefQuery(obj));
                break;        
            default:
                break;
            }
    ...
    void ListOfAllMembersQuery::foreachMember(const MemberRefQueryFunction& invoke) const
    {
        QueryForEach<MemberRefQuery, 63>(invoke, _node);
    }


    const String& ListOfAllMembersQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void ListOfAllMembersQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
