...
{
    void DescriptionQuery::visit(Visitors::DescriptionQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
    ...
    const String& DescriptionQuery::getTitle(const String& notFound) const
    {
        if (_node) {
	...
	void DescriptionQuery::getInternal(InternalQuery &dest) const
	{
		if (_node) {
	...
	InternalQuery DescriptionQuery::getInternal() const
	{
		InternalQuery dest;
		getInternal(dest);
		return dest;
    ...
    void DescriptionQuery::foreachParagraph(const ParaQueryFunction& invoke) const
    {
        QueryForEach<ParaQuery, 68>(invoke, _node);
    }
    void DescriptionQuery::foreachSect1(const Sect1QueryFunction& invoke) const
    {
        QueryForEach<Sect1Query, 90>(invoke, _node);
    }


    const String& DescriptionQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void DescriptionQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
