...
{
    const String& FormulaQuery::getId(const String& notFound) const
    {
        if (_node)
    ...
    const String& FormulaQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void FormulaQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
