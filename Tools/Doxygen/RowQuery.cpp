...
{
    void RowQuery::visit(Visitors::RowQueryVisitor *visitor) const
    {
        if (!_node || !visitor)
            return;
        ...
        for (Xml::Node* obj : objects) {
            ...
            case 30:
                visitor->visitedEntry(EntryQuery(obj));
                break;        
            default:
                break;
            }
    ...
    void RowQuery::foreachEntry(const EntryQueryFunction& invoke) const
    {
        QueryForEach<EntryQuery, 30>(invoke, _node);
    }


    const String& RowQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void RowQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
