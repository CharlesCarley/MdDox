...
-------------------------------------------------------------------------------
*/
#include "MemberRefQuery.h"
#include "TypeFilter/DoxygenFilter.h"


namespace MdDox::Doxygen
{
    const String& MemberRefQuery::getRefId(const String& notFound) const
    {
        if (_node)
    ...
    const String& MemberRefQuery::getAmbiguityScope(const String& notFound) const
    {
        if (_node)
            return _node->attribute("ambiguityscope", notFound);
        return notFound;
    }

	DoxProtectionKindEnum MemberRefQuery::getProt() const
	{
		if (_node)
	...
	DoxVirtualKindEnum MemberRefQuery::getVirt() const
	{
		if (_node)
    ...
    const String& MemberRefQuery::getScope(const String& notFound) const
    {
        if (_node) {
            Xml::Node *node = _node->firstChildOf(88);
            if (node)
                return node->text();
        }
        return notFound;
    ...
    const String& MemberRefQuery::getName(const String& notFound) const
    {
        if (_node) {
    ...
    const String& MemberRefQuery::text(const String &def) const
    {
        return _node ? _node->text() : def;
    }
    
    void MemberRefQuery::sort(const SortMap& childOrder) const
    {
        if (_node) {
            _node->sort(
